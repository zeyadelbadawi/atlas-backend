...
describe('LMS Authoring (e2e) — Phase 4', () => {
  ...
  it('Owner: full quiz CRUD, and correct answers never leak pre-submission', async () => {
    ...
    const studentSafe = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(
      studentSafe.body.questions.every((q: { options: unknown[] }) =>
  ...
  it('rejects malformed question shapes (wrong correct-option counts)', async () => {
  ...
  it('Instructor: can author quizzes/assignments only for a course they are assigned to', async () => {
  ...
  it('a total stranger cannot author quiz/assignment content for a course they have no relationship to', async () => {
  ...
  it('an Academy instructor (roster member, not course-assigned) cannot author quiz/assignment content', async () => {
  ...
  it('Full round trip: author -> publish -> enroll -> take quiz -> auto-scored result', async () => {
  ...
  it('Full round trip: author assignment -> publish -> student uploads a real file and submits -> instructor grades', async () => {
  ...
  it('rejects a submission-attachment upload from a student who is not actively enrolled', async () => {
    ...
    await request(app.getHttpServer())
      .post(
        `/courses/${course.id}/assignments/${assignment.body.id}/submission/attachment`,
      )
  ...
  it('assignment authoring list/detail return every status (draft included), unlike the student-safe published-only read', async () => {
