...
/**
 * P4 — `plans`/`add_ons` are platform-owned (no RLS), but still seeded via
 * the admin connection for consistency: there is no write endpoint for
 * either.
 *
 * Real-bug fix (checkout/plans investigation): every plan this helper
 * creates deliberately leaves `displayOrder` at its schema default (`0`)
 * and — as of this fix — defaults to a real, usable `pricing` object.
 * Both matter, for two different reasons:
 *
 *   - `displayOrder: 0` is what `PlansRepository`'s `CUSTOMER_FACING_WHERE`
 *     filter uses to keep every test-fixture plan out of `GET /plans`/the
 *     real Plans page/the default-trial-plan lookup, permanently, with no
 *     per-test cleanup required — see that constant's own doc comment for
 *     the full incident this closes. Do NOT give a fixture plan a non-zero
 *     `displayOrder`; that would make it customer-facing.
 *   - A real `pricing` default (previously absent entirely — every caller
 *     of this helper produced an unpriced plan by construction) means a
 *     test exercising real Checkout against a fixture plan gets genuinely
 *     realistic behavior instead of an accidental `pricingUnavailable`
 *     nobody intended. A test that specifically needs to exercise the
 *     unpriced-rejection path itself should pass `pricing: null` — see
 *     `checkout.service.e2e-spec.ts`'s own such case for the pattern.
 */
