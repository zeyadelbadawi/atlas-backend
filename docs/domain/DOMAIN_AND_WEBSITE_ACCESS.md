...
| P63g hardening in production | DEPLOYED 18 Sep 2026 (backend `5d5fb7f`, frontend `695be21`): migration `20261007040000_p63g_domain_hardening` applied (95 migrations); readiness reports the sweep block and a zero release backlog; HSTS `includeSubDomains` on the platform tree; a rejected CORS origin returns 200 without an allow header; the frontend deploy ran in `--frontend-only` mode and the backend deploy recorded last-good digests with both health gates green |
...
| End-to-end custom domain on a real customer hostname | PRODUCTION VERIFIED 18 Sep 2026 — `https://rawc.ae/` returns 200 with the Atlas app (was 525); the operator check records `https_status_code = 200`; Cloudflare then issued the custom-hostname certificate (`ssl_status = active`), so the row is live AND settled, `customLive = 1`, canonical host `rawc.ae`. Between the origin fix and certificate issuance the domain was correctly shown as "Live — Atlas certificate pending" because the customer's own Cloudflare zone terminates TLS. |
